...
st.title("🗣️ 英文發音 (Edge-TTS 版)")

# 選擇語音 (這兩個是微軟最好聽的英文聲音)
voice_option = st.selectbox("選擇聲音", [
    "en-US-AnaNeural",    # 美式女聲 (很自然)
    "en-US-GuyNeural",    # 美式男聲
    "en-GB-SoniaNeural"   # 英式女聲
])

text = st.text_input("輸入英文", "Hello! This voice sounds much better than Google.")
...
if st.button("播放"):
    if text:
        with st.spinner("生成聲音中..."):
            # 執行非同步函式
            mp3_fp = asyncio.run(generate_audio(text, voice_option))
