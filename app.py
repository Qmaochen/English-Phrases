...
if "requirements.txt" in files:
    st.success("✅ 找到了 requirements.txt！")
    # 讀取內容給你看
    with open("requirements.txt", "r") as f:
        content = f.read()
    st.text_area("requirements.txt 的內容是：", content)
else:
    st.error("❌ 找不到 requirements.txt！伺服器根本沒看到它！")
...
try:
    ...
    installed_packages = reqs.decode("utf-8")
    
    if "gTTS" in installed_packages or "gtts" in installed_packages:
        st.success("✅ gTTS 已經安裝成功！")
    else:
        st.error("❌ gTTS 沒有安裝！(雖然檔案可能在，但 pip install 沒跑)")
        
    with st.expander("查看所有已安裝套件"):
        st.text(installed_packages)
except Exception as e:
    st.error(f"檢查失敗: {e}")

# 3. 嘗試匯入
st.header("3. 嘗試 Import")
try:
    from gtts import gTTS
    st.success("🎉 Import 成功！現在可以用 from gtts import gTTS 了")
except ImportError as e:
    st.error(f"💀 Import 失敗: {e}")
except Exception as e:
    st.error(f"💀 其他錯誤: {e}")
